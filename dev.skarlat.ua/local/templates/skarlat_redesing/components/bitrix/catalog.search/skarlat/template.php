...
if (Loader::includeModule('search'))
{
	...
	{
		if (is_array($arElements) || empty($_GET['q']))
		{
			?>
            <div class="container">
                <div class="search-block-empty 2">
                    <h1><?=GetMessage("EMPTY_RESULT_1")?> "<?=$_GET['q']?>" <?=GetMessage("EMPTY_RESULT_2")?></h1>
                </div>
            </div>
			<?
			return;
...
if (!empty($searchFilter) && is_array($searchFilter))
{
    ...
    $arFilter = array("IBLOCK_ID" => $arParams['IBLOCK_ID'], "ID" => $searchFilter['ID'], "ACTIVE" => "Y");
    ...
    $searchFilter['ID'] = array_diff($yes,array_uintersect($no, $yes, "strcasecmp"));
	...
	if($_REQUEST['compare_small']=="Y") $componentParams["compare_item"]="Y";
