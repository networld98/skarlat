...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <? if($arResult['MODE_CLASSES'] <> ''): ?> <?= $arResult['MODE_CLASSES'] ?><? endif?>">
		...
		<div class="<?=$dropDownBlock?> bx-ui-sls-input-block" data-id="<?=$arResult['LOCATION']['ID']?>" >
			<span class="dropdown-icon"></span>
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" required="required" onchange="getNPOffices($(this).val());" id="autocompleteCity" value="<?=$arResult['LOCATION']['DISPLAY']?>" class="order-block__delivery-autocomplete" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?> ..." />
...
<?if($arParams['POST_CITY']!='Y'){?>
    <script>
        $(document).ready(function (){
            getNPOffices(<?=$arParams['ID']?>,"<?=$arParams['NP']?>");
        });
    </script>
<?}?>
	<script>
		...
		<?endif?>
        $('.order-block__delivery-autocomplete').val($('.bx-ui-sls-fake').val());
