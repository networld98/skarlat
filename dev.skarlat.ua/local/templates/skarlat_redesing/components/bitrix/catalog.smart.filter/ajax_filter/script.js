function JCSmartFilter(ajaxURL, viewMode, params)
{
	...
	if (params && params.SEF_DEL_FILTER_URL)
	{
		...
		this.bindUrlToButton('del_filter_modal', params.SEF_DEL_FILTER_URL);
	}
}
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		if (!!modef && !!modef_num)
		{
			...
			if (result.INSTANT_RELOAD && $(window).width() >991 && result.COMPONENT_CONTAINER_ID)
			{
				url = BX.util.htmlspecialcharsback(result.FILTER_AJAX_URL);
				BX.ajax.insertToNode(url, result.COMPONENT_CONTAINER_ID);
			}
			...
			{
				//Если модальный фильтр не открыт обновляем страницу
...
{
	var widthSelect = $('.filters-block .select-selected#el'+popupId).parent('div').width();
	...
	$("#smartFilterDropDown"+popupId).width(widthSelect);
};
