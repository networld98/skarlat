...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		'CURRENCY' => $row['CURRENCY_FORMATED'],
		...
		'BRAND' => isset($row[$this->arParams['BRAND_PROPERTY'].'_VALUE'])
			...
			: '',
        'SHORT_CATEGORY_NAME' => $row['PROPERTY_SHORT_CATEGORY_NAME_VALUE']
	);
    ...
    $file = new IO\File(Application::getDocumentRoot() . $rowData['IMAGE_URL']);
    $isExist = $file->isExists();
    if($isExist == false){
        $rowData['IMAGE_URL'] = SITE_TEMPLATE_PATH."/img/no_photo.png";
    }
...
$totalData = array(
    ...
    'PRICE_WITHOUT_DISCOUNT' => $result['allSum']+$result['DISCOUNT_PRICE_ALL'] ,
