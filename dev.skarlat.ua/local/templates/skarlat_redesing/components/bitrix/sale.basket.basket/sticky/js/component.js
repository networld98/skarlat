function orderDeliveryCalc(){
		if($('.delivery-val:checked').length>0){
			var price=$('.delivery-val:checked').data('price');
			if(price<1){
				price="<small>Уточняйте у менеджера</small>";
				var baseprice=$('#fullprice').val();
				$('#fullprice_formated').html(baseprice);
			}
			else{
				var fullprice;
				var baseprice=$('#fullprice').val();
				fullprice=Math.round(baseprice)+Math.round(price);
				$('#fullprice_formated').html(fullprice);
			}
...
;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		init: function(parameters)
		{
			...
			this.getCacheNode("footRoot").style.opacity = 1;
		...
		checkStickyHeaders: function()
		{
			...
			var basketPosition = BX.pos(this.getCacheNode("footRoot"));
			var basketScrolledToEnd = scrollTop + 200 >= basketPosition.bottom;

			if (BX.util.in_array('top', this.params.TOTAL_BLOCK_DISPLAY))
			{
				var totalBlockNode = this.getEntity(this.getCacheNode("footRoot"), 'basket-total-block');
		...
		editTotal: function()
		{
			...
			setTimeout(function(){orderDeliveryCalc();},500);
		},

		fillTotalBlocks: function()
		{
			var totalNodes = this.getEntities(this.getCacheNode("footRoot"), 'basket-total-block');
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					if(result.BASKET_DATA.BASKET_ITEMS_COUNT<1) window.location.reload();
					...
					$('#fullprice').val(result.BASKET_DATA.allSum);
			...
			new BX.easing({
					...
					for (var nodeId in animationData.start)
					{
						if (animationData.start.hasOwnProperty(nodeId))
						{
							if (!nodeCache[nodeId])
							{
								...
								{
									nodeCache[nodeId] = this.getEntities(this.getCacheNode("footRoot"), this.ids.total);
								}
								else
								{
									var node = BX(nodeId);
							...
							{
								nodeCache[nodeId][i].innerHTML = state[nodeId];
							}
						}
					}
				}, this),
				complete: BX.delegate(function() {
					...
					for (nodeId in animationData.start)
					{
						if (animationData.start.hasOwnProperty(nodeId))
						{
							formattedPrice = animationData.finish[nodeId];
			...
			if (startPrice !== finalPrice)
			{
				...
				new BX.easing({
					...
					step: BX.delegate(function(state){
						...
						priceNode.innerHTML = state.price;
					}, this),
					complete: BX.delegate(function() {
						var node, price;
