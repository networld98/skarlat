...
{
    $arPrice = CPrice::GetByID($row['PRODUCT_PRICE_ID']);
    $prop=CIBlockElement::GetByID($row['PRODUCT_ID'])->GetNextElement()->GetProperties();
    $minCont = $prop['MIN_ORDER']['VALUE'];
    $currencyList = \Bitrix\Currency\CurrencyManager::getCurrencyList();
    if( preg_match( '!\(([^\)]+)\)!', $currencyList[$arPrice['CURRENCY']], $match ) )
	...
	$rowData = array(
		...
		'CURRENCY_FORMATED_ITEM' =>  $currencyName,
        'CURRENCY_FORMATED' =>  $arCurres[$row['CURRENCY']],
		...
		'SUM_FULL_RIGHT_PRICE_FORMATED' => CurrencyFormat($row['SUM_FULL_PRICE']-$row['SUM_DISCOUNT_PRICE'],$row['CURRENCY']),
		'SUM_FULL_RIGHT_PRICE' => $row['SUM_FULL_PRICE']-$row['SUM_DISCOUNT_PRICE'],
