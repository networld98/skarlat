...
$arSelect = Array("ID", "NAME", "IBLOCK_ID","DETAIL_TEXT","IBLOCK_SECTION_ID","DETAIL_PICTURE","PREVIEW_PICTURE", "DETAIL_PAGE_URL","CATALOG_QUANTITY","PROPERTY_SHORT_CATEGORY_NAME");
...
while($ob = $res->GetNextElement()) {
    ...
    if(!in_array($arFields['IBLOCK_SECTION_ID'],(array)$arSections) && $arFields['CATALOG_QUANTITY']){
        ...
        foreach ($arProps['MORE_PHOTO']['VALUE'] as $pict):
            if ($i == 10) break;
            if ($pict < 1) continue;
            $medias = $dom->createElement("picture", "https://". $site_dir . CFile::GetPath($pict));
            $element->appendChild($medias);
        endforeach;
...
$dom->save($_SERVER["DOCUMENT_ROOT"]."/upload/yml_catalogUa.xml");
print "Обновлен";