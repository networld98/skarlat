...
if($_POST['discountList']=='Y'){
    ...
    {
        $account = $arAccountCurrency;
    }?>
    <td width="60%" class="adm-detail-content-cell-l" style="text-align: left">
        <a id="affiliate" <?if($account['ID']!=NULL){?>disabled="" <?}?> class="adm-btn <?if($account['ID']!=NULL){?>disabled<?}?>" title="Сгенерировать партнерский код">Сгенерировать партнерский код</a>
        <?if($account['ID']!=NULL){?>
            <a href="/bitrix/admin/sale_account_edit.php?ID=<?=$account['ID']?>&lang=ua">Просмотреть счёт</a>
        <?}?>
    </td>
    <?if($account['ID'] != NULL && $_POST['coupon']==NULL){?>
        <script>
            $("input[name='UF_COUPON']").val('<?=$account['NOTES']?>');
        </script>
    <?}?>
    <?if($account['ID'] != NULL && $_POST['coupon']==NULL){?>
        <script>
