...
<?if(empty($_POST['sum'])){?>
    <div class="info">
        <span>You have on your balance</span>
    ...
    <form id="form_clinic" name="form_clinic" action="" method="post">
        ...
        <div class="row">
            <div class="col-lg-6">
                ...
                <input type="number" max="<?echo round(CSaleUserAccount::GetByUserID($USER->GetID(), "UAH")['CURRENT_BUDGET'],0);?>"  name="sum">
            </div>
            <div class="col-lg-6">
                <label>Output method:</label>
                <div class="radio-block">
                    <label for="way"><input type="radio" name="way" checked value="На карту"/><span>Card Payment</span></label>
                    <label for="way"><input type="radio" name="way" value="Наличка"/><span>Cash</span></label>
                </div>
            </div>
            <div class="col-lg-12 card-client-block">
                <label for="sum">Credit card number:</label>
                <input type="text" name="clientCard">
            </div>
        </div>
        <button class="primary" type="submit" name="saveProfile">Request</button>
    </form>
    <script>
...
<?}else {
    ...
    $arLoadProductArray = array(
        ...
        "NAME" => "Запрос на списание (" . $_POST['userId'] . ") " . date("d.m.y"),
        "ACTIVE" => "N",
    );
    ...
    <div class="info" style="padding: 10px 10px 40px;line-height: 1.5;">
        <h6>
            Withdrawal request <?=$_POST['sum'];?>грн. (<?=$way?>) sent to manager.
        </h6>
    </div>
<?}?>